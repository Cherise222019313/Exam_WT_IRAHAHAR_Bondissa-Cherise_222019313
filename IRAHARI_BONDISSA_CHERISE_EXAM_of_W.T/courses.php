...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO courses(courseID, course_name, instructorID) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $courseID, $course_name, $instructorID);

    // Set parameters and execute
    $courseID = $_POST['Event_ID'];
    $course_name = $_POST['Event_Name'];
    $instructorID = $_POST['Description'];
...
<body>
    <center><h2>Courses Table</h2></center>
    <table border="3">
        <tr>
            <th>courseID</th>
            <th>course_name</th>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $courseID = $row['courseID']; // Fetch the courseID
        echo "<tr>
            <td>" . $row['courseID'] . "</td>
            <td>" . $row['course_name'] . "</td>
            <td>" . $row['instructorID'] . "</td>
            <td><a style='padding:4px' href='delete_courses.php?courseID=$courseID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_courses.php?courseID=$courseID'>Update</a></td> 
        </tr>";
    }
