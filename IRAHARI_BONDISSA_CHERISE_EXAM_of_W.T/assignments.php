...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO assignments(assignmentID, courseID, title, description, due_date) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("issss", $assignmentID, $courseID, $title, $description, $due_date);

    // Set parameters and execute
    $assignmentID = $_POST['Event_ID'];
    $courseID = $_POST['Event_Name'];
    $title = $_POST['Description'];
    $description = $_POST['Start_Date'];
...
<body>
    <center><h2>Assignments Table</h2></center>
    <table border="3">
        <tr>
            <th>assignmentID</th>
            <th>courseID</th>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $assignmentID = $row['assignmentID']; // Fetch the assignmentID
        echo "<tr>
            ...
            <td>" . $row['due_date'] . "</td>
            <td><a style='padding:4px' href='delete_assignments.php?assignmentID=$assignmentID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_assignments.php?assignmentID=$assignmentID'>Update</a></td> 
        </tr>";
    }
