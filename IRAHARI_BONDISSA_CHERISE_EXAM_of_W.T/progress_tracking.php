...
<head>
  ...
  <title>progress_tracking Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
<section>
   <h1><u>Progress_tracking Form</u></h1>


<form method="post" onsubmit="return confirmInsert();">

    <label for="progressID">ProgressID:</label>
    ...
    <label for="completion_status">completion_status:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO progress_tracking(progressID, userID, courseID, moduleID, completion_status,completion_date) VALUES (?, ?, ?, ?, ?,?)");
    $stmt->bind_param("issss", $progressID, $userID, $courseID, $moduleID, $completion_status,$completion_date);

    // Set parameters and execute
    $progressID = $_POST['Event_ID'];
    $userID = $_POST['Event_Name'];
    $courseID = $_POST['Description'];
    $moduleID = $_POST['Start_Date'];
    $completion_status = $_POST['End_Date'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>progress_tracking DETAILS</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Progress_tracking Table</h2></center>
    <table border="3">
        <tr>
            <th>progressID</th>
            <th>userID</th>
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_progress_tracking.php?progressID=$progressID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_progress_tracking.php?progressID=$progressID'>Update</a></td> 
        </tr>";
    }
